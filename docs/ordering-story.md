...
family stores a node's children in a Go map and returns them alphabetically, so
a model with leaves `z`, `m`, `a` can come out as `a`, `m`, `z` — a design
suited to lookup and analysis rather than order-faithful serialization.

Cambium targets that order-sensitive use case by making order a structural
property of the tree. It delegates sibling ordering to libyang and reads
children from the `lyd_node.next`/`prev` chain, not from a keyed map.
