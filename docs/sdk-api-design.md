...
> finish-line audit use [`docs/release-readiness-2026-06-20.md`](release-readiness-2026-06-20.md).
...
## C. Data tree CRUD + navigation  *(NEW — data-tree layer beyond goyang's scope)*
...
The user-ordered type stays positional-only (no order-agnostic setter); gains a read side and a leaf-list sibling.
...
The highest-value net-new layer, beyond goyang's scope. Depends on Phase 0 (handle model) and Phase 1 (rich schema for the data->schema bridge). Validation-with-structured-errors is a feature beyond ygot's scope (no must/when/mandatory).
