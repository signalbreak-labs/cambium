...
> ⚠️ **STATUS — HISTORICAL.** This is the original design brief and predates a major scope change; it is kept for design intent, not current state.
> **2026-06-20: the Rust stack was REMOVED — Cambium is now Go-only.** The removed Rust code lives in git history.
> The shared contract (`/spec`, `/conformance`, `/VERSIONS`) is deliberately kept language-neutral so a Rust (or other) binding can return as a first-class peer under `/<lang>/` (see AGENTS.md, "Adding a language binding").
> For **current** status, defer to `AGENTS.md` and `/spec` — not this doc. References below to "Rust (primary)", Rust crates, and "two runners" are historical.
...
- **(A) Canonical-only — adopted.** `ordered-by user` byte-exact; system-ordered deterministic-canonical (stable across runs, languages, *and* device-return-order). RFC-correct.
...
### 1.4 goyang / ygot design gaps Cambium targets
...
```
```go
// NOTE: data-tier parse/validate/serialize is the OPTIONAL libyang backend
// (requires cgo) — import "github.com/signalbreak-labs/cambium/go/libyangbackend".
// The default cgo-free `package cambium` is Schema-IR + codegen only and has no
// Parse/Validate/Serialize. See spec/ordering-invariants.md §0.
ctx, _ := backend.NewContextBuilder().SearchPath("yang").LoadModule("openconfig-interfaces", nil).Freeze()
t, _ := backend.Parse(ctx, backend.FormatJSON, backend.ParseModeDataOnly, bytes)
_ = t.Validate(backend.ValidateModeConfigOnly | backend.ValidateModeMultiError)
xml, _ := t.Serialize(backend.FormatXML, backend.SerializeWithSiblings)
```
