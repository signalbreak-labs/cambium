...
func validateRevisionStatementChildren(stmt *yangparse.Statement) error {
	for _, child := range stmt.SubStatements() {
		if err := validateNestedRevisionStatementChildPlacement(stmt, child); err != nil {
			return err
		}
	}
	return nil
...
func validateDependencyStatementChildren(stmt *yangparse.Statement) error {
	for _, child := range stmt.SubStatements() {
		if err := validateNestedDependencyStatementChildPlacement(stmt, child); err != nil {
			return err
		}
	}
	return nil
...
func validateBelongsToStatementChildren(stmt *yangparse.Statement) error {
	for _, child := range stmt.SubStatements() {
		if err := validateNestedBelongsToStatementChildPlacement(stmt, child); err != nil {
			return err
		}
	}
	return nil
...
func validateYangVersionStatementChildren(stmt *yangparse.Statement) error {
	for _, child := range stmt.SubStatements() {
		if err := validateNestedYangVersionStatementChildPlacement(stmt, child); err != nil {
			return err
		}
	}
	return nil
