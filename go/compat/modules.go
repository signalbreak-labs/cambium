...
var moduleSchemas sync.Map
var moduleEntries sync.Map

func setModuleSchema(m *Module, schema cambium.Module) {
	if m != nil {
		moduleSchemas.Store(m, schema)
	}
}
...
func moduleSchema(m *Module) (cambium.Module, bool) {
	...
	value, ok := moduleSchemas.Load(m)
	if !ok {
		return cambium.Module{}, false
	}
	schema, ok := value.(cambium.Module)
	return schema, ok
}

func setModuleEntry(m *Module, entry *Entry) {
	if m != nil && entry != nil {
		moduleEntries.Store(m, entry)
	}
}
