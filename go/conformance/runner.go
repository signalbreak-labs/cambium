...
// Package conformance is the Go side of Cambium's shared /conformance corpus.
// It reads the same manifest.toml the Rust runner uses, parses each fixture, and
// asserts byte-for-byte equality with the golden outputs (after the same
// trailing-whitespace normalization the Rust runner applies).
package conformance
...
func RunCase(conformanceDir string, c Case) error {
	...
	if c.OpType != "" {
		...
		tree, err = ctx.Parse(inFmt, cambium.ParseModeDataOnly, input)
		if err != nil {
			return err
		}
	}
...
func loadModulesInDir(ctx *cambium.Context, dir string) error {
...
func runYanglintOracle(yanglint, moduleDir, inputPath string, format cambium.Format, wd cambium.WithDefaults, opType string) ([]byte, error) {
...
func yanglintFormatArg(format cambium.Format) (string, error) {
	switch format {
	case cambium.FormatXML:
		return "xml", nil
	case cambium.FormatJSON, cambium.FormatJSONIETF:
		return "json", nil
	default:
...
func yanglintWithDefaultsArg(wd cambium.WithDefaults) string {
	switch wd {
	case cambium.WithDefaultsTrim:
		return "trim"
	case cambium.WithDefaultsAll:
		return "all"
	case cambium.WithDefaultsAllTagged:
		return "all-tagged"
	default:
		return ""
	}
...
func parseFormat(s string) (cambium.Format, error) {
	switch strings.ToLower(s) {
	case "xml":
		return cambium.FormatXML, nil
	case "json":
		return cambium.FormatJSON, nil
	case "json-ietf", "json_ietf":
		return cambium.FormatJSONIETF, nil
	case "lyb":
		return cambium.FormatLYB, nil
	default:
		return 0, fmt.Errorf("unknown format: %s", s)
	}
}

func parseWithDefaults(s string) (cambium.WithDefaults, error) {
	switch strings.ToLower(s) {
	case "explicit":
		return cambium.WithDefaultsExplicit, nil
	case "trim":
		return cambium.WithDefaultsTrim, nil
	case "all", "report-all":
		return cambium.WithDefaultsAll, nil
	case "all-tagged", "report-all-tagged":
		return cambium.WithDefaultsAllTagged, nil
	default:
		return 0, fmt.Errorf("unknown serialize-defaults: %s", s)
	}
}

func parseOpType(s string) (cambium.OpType, error) {
	switch strings.ToLower(s) {
	case "rpc":
		return cambium.OpTypeRPC, nil
	case "notification":
		return cambium.OpTypeNotification, nil
	case "reply":
		return cambium.OpTypeReply, nil
	default:
		return 0, fmt.Errorf("unknown op-type: %s", s)
	}
}

func formatBytesForCompare(format cambium.Format, b []byte) []byte {
	if format == cambium.FormatLYB {
		return b
	}
	return normalize(b)
}
