...
func (t *RawDataTree) Serialize(format Format, options uint32) ([]byte, error) {
	...
	defer runtime.KeepAlive(t.owner)
	if format == FormatLYB {
		return t.SerializeLYB(options)
	}
	if format == FormatJSONIETF {
		options |= PrintEmptyCont
	}
	var out *C.char
	rc := C.lyd_print_mem(&out, t.tree, format.c(), C.uint32_t(options))
	if rc != C.LY_SUCCESS {
		return nil, lyError(t.owner.ctx, "serialize", int(rc))
	}
	if out == nil {
...
// SerializeLYB prints the whole tree to LYB bytes using the length-aware output
// handler. LYB may contain embedded NULs, so it cannot use lyd_print_mem.
func (t *RawDataTree) SerializeLYB(options uint32) ([]byte, error) {
	...
	if rc != C.LY_SUCCESS {
		return nil, lyError(t.owner.ctx, "ly_out_new_memory", int(rc))
	}
	first := C.lyd_first_sibling(t.tree)
	rc = C.lyd_print_all(out, first, C.LYD_LYB, C.uint32_t(options))
	if rc != C.LY_SUCCESS {
		C.ly_out_free(out, nil, 1)
		return nil, lyError(t.owner.ctx, "lyd_print_all", int(rc))
	}
...
func (d *RawDataDiff) Serialize(format Format, options uint32) ([]byte, error) {
	...
	if format == FormatLYB {
		return d.SerializeLYB(options)
	}
	if format == FormatJSONIETF {
		options |= PrintEmptyCont
	}
	var out *C.char
	rc := C.lyd_print_mem(&out, d.tree, format.c(), C.uint32_t(options))
	if rc != C.LY_SUCCESS {
		return nil, lyError(d.ctx, "serialize", int(rc))
	}
	if out == nil {
		return nil, nil
	}
...
// SerializeLYB prints the diff tree to LYB bytes using the length-aware output
// handler. LYB may contain embedded NULs, so it cannot use lyd_print_mem.
func (d *RawDataDiff) SerializeLYB(options uint32) ([]byte, error) {
	...
	rc := C.ly_out_new_memory(&buf, 0, &out)
	if rc != C.LY_SUCCESS {
		return nil, lyError(d.ctx, "ly_out_new_memory", int(rc))
	}
	first := C.lyd_first_sibling(d.tree)
	rc = C.lyd_print_all(out, first, C.LYD_LYB, C.uint32_t(options))
	if rc != C.LY_SUCCESS {
		C.ly_out_free(out, nil, 1)
		return nil, lyError(d.ctx, "lyd_print_all", int(rc))
	}
	length := C.ly_out_printed(out)
	var b []byte
	if length > 0 {
