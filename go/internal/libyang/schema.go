...
func extractRange(rangePtr *C.struct_lysc_range, kind rangeKind, fractionDigits uint8) []RawRangeBound {
	...
	for i := C.uint64_t(0); i < count; i++ {
		...
		case rangeDecimal64:
			minStr = formatDecimal64(int64(C.cam_range_part_min64(part)), fractionDigits)
			maxStr = formatDecimal64(int64(C.cam_range_part_max64(part)), fractionDigits)
		}
...
func formatDecimal64(raw int64, fractionDigits uint8) string {
	if fractionDigits == 0 {
