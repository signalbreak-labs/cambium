...
//
// Modified by signalbreak-labs for Cambium: adjusted unquoted-argument comment
// boundary handling and tightened lexer diagnostics/escape validation.
...
func lexQString(l *lexer) stateFn {
	...
	for {
		...
		case '\\':
			...
			case '\\':
			default:
				// YANG double-quoted strings only define \n, \t, \",
				// and \\. Pattern regex escapes must be represented as
				// escaped backslashes in the YANG source.
				l.ErrorfAt(l.line, l.col-2, "%s", `invalid escape sequence: \`+string(c))
				text = append(text, '\\')
			}
			fallthrough
		default:
	...
	for {
		...
		case '*':
			if strings.HasPrefix(l.input[l.pos:], "*/") {
				l.Errorf(`invalid comment sequence "*/" in unquoted string`)
				return lexGround
			}
			l.next()
		default:
