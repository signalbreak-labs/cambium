...
func TestUserOrderedViewRead(t *testing.T) {
	...
	// G-19: a duplicate query key name must NOT inflate the match count into a
	// false positive (the old impl returned (1, true) here).
	if _, ok, _ := view.FindByKey([][2]string{{"name", "a"}, {"name", "a"}}); ok {
		t.Fatal("FindByKey with a duplicate query key name must return ok=false")
	}
}
